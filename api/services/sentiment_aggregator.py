...
Sources:
  - News sentiment:      narrative_signals.direction × confidence (Phase 1)
  - Social sentiment:    StockTwitsAdapter
  - Narrative momentum:  narrative velocity (normalised)

Composite formula:
    0.4 × news_sentiment + 0.3 × social_sentiment + 0.3 × narrative_momentum
...
class SentimentAggregator:
    ...
    def _news_sentiment(self, ticker: str) -> float:
        ...
        try:
            ...
            logger.warning("SentimentAggregator._news_sentiment: signal fetch failed: %s", type(exc).__name__)
            return 0.0
        ...
        for sig in signals:
            try:
                ...
                logger.warning("SentimentAggregator._news_sentiment: signal row failed: %s", type(exc).__name__)
                continue
    ...
    def _narrative_momentum(self, ticker: str) -> float:
        ...
        try:
            ...
            logger.warning("SentimentAggregator._narrative_momentum: narrative fetch failed: %s", type(exc).__name__)
            return 0.0
        ...
        for nar in narratives:
            try:
                ...
                logger.warning("SentimentAggregator._narrative_momentum: narrative row failed: %s", type(exc).__name__)
                continue
    ...
    def compute_ticker_sentiment(self, ticker: str) -> dict:
        ...
        try:
            ...
            logger.warning("SentimentAggregator: spike detection failed for %s: %s", ticker, type(exc).__name__)
    ...
    def compute_market_sentiment(self, tickers: list[str]) -> dict:
        ...
        for ticker in tickers:
            try:
                ...
                logger.warning("SentimentAggregator: compute_ticker_sentiment failed for %s: %s", ticker, type(exc).__name__)
                results.append({
