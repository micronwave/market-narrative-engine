...
# Realistic parser path with mixed sentiment payload.
payload_ok = {
    "messages": [
    ...
    "symbol": {"watchlist_count": 12000},
}
with patch("urllib.request.urlopen", return_value=_FakeUrlOpenResponse(payload_ok)):
    parsed = adapter.get_sentiment("AAPL")
T("parses mixed bullish/bearish/neutral counts",
  parsed is not None and parsed.get("bullish_count") == 2 and parsed.get("bearish_count") == 1 and parsed.get("neutral_count") == 1,
  f"got={parsed}")
T("sets trending from realistic symbol metadata",
  parsed is not None and parsed.get("trending") is True,
  f"got={parsed}")

# Malformed provider payload branch.
payload_bad = {"messages": "not-a-list", "symbol": {"watchlist_count": 1}}
adapter_bad = StockTwitsAdapter()
with patch("urllib.request.urlopen", return_value=_FakeUrlOpenResponse(payload_bad)):
    malformed = adapter_bad.get_sentiment("MSFT")
T("returns None for malformed payload shape", malformed is None, f"got={malformed!r}")
...
mock_repo_trending = MagicMock()
mock_repo_trending.get_trending_tickers.return_value = [
    {"ticker": "AAPL", "total_mentions": 100},
    {"ticker": "MSFT", "total_mentions": 80},
]
...
mock_repo_leaderboard = MagicMock()
mock_repo_leaderboard.get_all_narrative_signals.return_value = [
    {"narrative_id": "n1", "direction": "bullish", "confidence": 0.8, "magnitude": "significant", "certainty": "expected", "catalyst_type": "earnings"},
    {"narrative_id": "n2", "direction": "neutral", "confidence": 0.9, "magnitude": "significant", "certainty": "expected", "catalyst_type": "macro"},
]
mock_repo_leaderboard.get_narrative.side_effect = lambda nid: {
    "n1": {"name": "Narrative 1", "stage": "Growing", "suppressed": 0},
    "n2": {"name": "Narrative 2", "stage": "Mature", "suppressed": 0},
}.get(nid)
with patch("api.app_legacy.get_repo", return_value=mock_repo_leaderboard):
...
if _fail > 0:
    try:
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(f"\ntest_sentiment_api.py: {_pass} passed, {_fail} FAILED\n")
    except Exception:
        pass
    sys.exit(1)
