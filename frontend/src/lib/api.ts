...
/** Fetch current user's portfolio and holdings. */
...
export async function fetchPortfolioExposure(): Promise<{ exposures: NarrativeExposure[]; total?: number; limit?: number; offset?: number; limited?: boolean }> {
...
export async function fetchPortfolioCorrelation(): Promise<CorrelationMatrix> {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      // ignore JSON parse failure and fall back to status-based message
    }
    if (res.status === 422 || res.status === 503) {
      throw new Error(detail || `portfolio correlation fetch failed: ${res.status}`);
    }
    throw new Error(`portfolio correlation fetch failed: ${res.status}`);
  }
  return res.json();
}
...
/** Per-ticker sentiment snapshot (news + social + momentum). */
...
/** Time-window sentiment history for one ticker. */
